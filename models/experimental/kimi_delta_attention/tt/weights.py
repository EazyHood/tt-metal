...
def _cache_stem(cache_name_prefix: str, name: str, tensor_parallel_size: int, tensor_parallel_axis: int) -> str:
    return f"{cache_name_prefix}.{name}.tp{tensor_parallel_size}.axis{tensor_parallel_axis}"
...
class KDAWeights:
    ...
    def check_cache_complete(
        ...
        *,
        ...
        _, tensor_parallel_size = _parallel_geometry(mesh_device, tensor_parallel_axis)
        for name in _cache_artifact_names(config):
            stem = _cache_stem(cache_name_prefix, name, tensor_parallel_size, tensor_parallel_axis)
    ...
    *,
    ...
    def device_tensor(
        ...
        *,
        ...
        cache_name = _cache_stem(cache_name_prefix, name, tensor_parallel_size, tensor_parallel_axis)
    ...
    if state_dict:
        ...
        for tap in range(config.conv_kernel_size):
            ...
            fused_tap = torch.cat(tap_weights).reshape(1, 1, config.q_dim + config.k_dim + config.v_dim)
            if tensor_parallel_size > 1:
