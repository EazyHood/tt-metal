...
ttnn::device_operation::ProgramArtifacts TransposeHCTiledProgramFactory::create_program_artifacts(
    ...
    // Donor writer forked into the op directory (legacy source lives under eltwise/unary; the
    // orchestration constraint forbids editing out-of-directory files, so it is forked here).
    KernelSpec writer{
    ...
    spec.work_units = {WorkUnitSpec{.name = "main", .kernels = {HCT_READER, HCT_WRITER}, .target_nodes = total_cores}};
