...
// Metal 2.0 fork of bmm_large_block_zm_fused_bias_activation.cpp.
//
// This is a deliberate `_metal2`-suffixed copy of the shared legacy compute kernel
// (bmm_large_block_zm_fused_bias_activation.cpp), created because the legacy source is still bound by
// the not-yet-ported factories (mcast_1d / mcast_2d / dram_sharded / batched_hs / sparse) via the
// ProgramDescriptor KernelDescriptor path, and those cannot co-migrate to Metal 2.0 yet (mcast_1d/2d
// are still on the imperative host_api.hpp builder; mcast_1d additionally uses GlobalCircularBuffer).
// See docs/.../metal2_port_patterns.md "Caution: Modifying a shared dataflow kernel" (fork option).
//
// The algorithm body is byte-for-byte identical to the legacy kernel. Only the host-binding surface
// is converted:
...
// construct CircularBuffer wrappers from them, which remains valid Device 2.0 on Gen1.
//
// SUNSET: delete this fork when the last legacy consumer of the original kernel ports to Metal 2.0,
// at which point the original can be Metal-2.0-ified in place and this copy removed.
// DRIFT: until then, any bug fix to the legacy original must be evaluated for this copy.
...
// RUNTIME ARGS
    ...
    // the file-scope reference to dfb::cb_in0_transposed below can be gated at the preprocessor level:
    // that DFB is only bound when the host emits the define, and Metal 2.0 only declares the dfb::
    // token for bound DFBs (see metal2_port_patterns.md "Conditional / optional DFB bindings").
