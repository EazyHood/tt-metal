...
inline std::string get_generic_reduction_doc(
    ...
    const char* int32_row = int32_supported ? R"doc(
                * - INT32
                  - TILE)doc"
                                            : "";
    // sum and mean expose fast_and_approximate_mode and output_layout; min and max expose neither,
    // so both entries are gated together — otherwise min/max would document arguments they reject.
