...
ttnn::device_operation::ProgramArtifacts TopKDeviceOperation::TopKSingleCoreProgramFactory::create_program_artifacts(
        ...
        .compile_time_args =
            {
                ...
                {"uint16_output", static_cast<uint32_t>(uint16_output)},  // Index format flag
            },
    ...
    KernelSpec compute{
        ...
        // The index output is UInt16 for narrow rows and UInt32 for wide ones; only the wide
        // case needs a 32-bit Dest register to carry an index through the sort intact.
        // double_buffer_dest is the inverse of the legacy dst_full_sync_en flag.
        .hw_config =
            ComputeGen1Config{
                .enable_32_bit_dest = !uint16_output,
