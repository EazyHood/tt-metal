...
| compute | `device/kernels/compute/topk.cpp` | `core_range` (`:249`) | 0-7 the eight CB indices `c_0`…`c_7`, 8 `Ht`, 9 `Wt`, 10 `Ktiles`, 11 `largest`, 12 `stable` (`:230-244`) | none | per core: 0 `work_per_core` (`:280-284`) | none | none | absent → resolves to **`O3`** (compute) | `ComputeConfigDescriptor{.fp32_dest_acc_en = !uint16_output, .dst_full_sync_en = false}` (`:251-254`) |
...
| compute | `ComputeConfigDescriptor{.fp32_dest_acc_en = !uint16_output, .dst_full_sync_en = false}`; `math_fidelity`, `math_approx_mode`, `bfp8_pack_precise`, `unpack_to_dest_mode` all left at their legacy defaults | `ComputeGen1Config{.enable_32_bit_dest = !uint16_output, .double_buffer_dest = true}` (Style B; the op sets a Metal `ComputeConfigDescriptor` directly, so the config is built directly and *not* routed through the TTNN helper, whose defaults differ) |
...
**`unpack_modes`: no entry is required or added.** The validator's forced-entry rule fires only when
a compute kernel consumes a `Float32`-formatted DFB while `enable_32_bit_dest` is true. This op
cannot reach that state: `validate_on_program_cache_miss` restricts the input dtype to `BFLOAT16` or
`BFLOAT8_B` (`device/topk_device_operation.cpp:168-171`) and the values output dtype equals the input
dtype, so the value-carrying DFB formats are only ever `Float16_b` or `Bfp8_b`, and the
index-carrying ones only `UInt16` / `UInt32`. Legacy set no `unpack_to_dest_mode` (every CB resolves
to `Default` → `UnpackToSrc`), which the empty table reproduces.
