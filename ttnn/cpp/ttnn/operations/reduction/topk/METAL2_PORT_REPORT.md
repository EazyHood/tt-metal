...
Identical. Most of those shapes route to the single-core factory (`multi_core_min_width` is 8192, and
...
`nightly/.../test_reduction_ops.py`, 5435 items) was run post-port only, and was still in flight when
this report was committed: 1378 items in with **0 failures**, including every `test_5d_topk` and
`test_graph_capture_topk` case reached so far. It was not baselined pre-port because it takes roughly
...
- **The `unpack_modes` forced-entry rule reads as always-applicable.** Working out that this op can
  *never* trip it took a detour: the rule fires only for a Float32 DFB under `enable_32_bit_dest`, and
  ruling that out meant tracing the op's dtype validation
  (`device/topk_device_operation.cpp:168-171`) through to every buffer's format. That is the right
  reasoning, but the section reads as though an entry is normally needed; leading with "first check
  whether any consumed DFB can be Float32 at all, for many ops the answer is no" would frame it
  better.
