...
void expect_half_grid_slots_packed(Program& program, uint32_t num_dfbs_per_half, bool is_quasar) {
    for (uint32_t i = 0; i < num_dfbs_per_half * 2; ++i) {
        ...
        EXPECT_EQ(dfb->id, i);
        if (is_quasar) {
            EXPECT_EQ(dfb->device_slot, i) << name;
        } else {
...
m2::KernelSpec make_touch_producer(const std::string& name, uint32_t num_dfbs, IDevice* device) {
    ...
    kernel.compile_time_args = {{"num_dfbs", num_dfbs}, {"implicit_sync", implicit_sync ? 1u : 0u}};
    if (!implicit_sync) {
...
m2::KernelSpec make_touch_consumer(
    ...
    kernel.compile_time_args = {
        {"num_dfbs", num_dfbs}, {"implicit_sync", implicit_sync ? 1u : 0u}, {"touched_magic", touched_magic}};
...
// (credit handshake on Gen1; config probe under Quasar's default implicit sync).
m2::ProgramSpec make_split_touch_spec(IDevice* device, uint32_t num_dfbs_per_half) {
    ...
    for (uint32_t i = 0; i < num_dfbs_per_half * 2; ++i) {
        ...
        if (i < num_dfbs_per_half) {
            bind_touch_dfbs(prod_a, cons_a, name, local);
        } else {
            bind_touch_dfbs(prod_b, cons_b, name, local);
        }
    }
...
TEST_F(MeshDeviceFixture, HalfGrid3Plus3DFBsOnDevice) {
    ...
    for (uint32_t i = 0; i < per_half * 2; ++i) {
        inputs.push_back(MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{}));
        outputs.push_back(MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{}));
    }
...
TEST_F(MeshDeviceFixture, HalfGrid16Plus16DFBsOnDevice) {
    ...
    expect_touch_results(device, CoreCoord{0, 0}, result_addr, per_half, /*entry_size=*/32, touched_magic, left_slots);
    expect_touch_results(device, CoreCoord{1, 0}, result_addr, per_half, /*entry_size=*/32, touched_magic, right_slots);
}
...
TEST_F(MeshDeviceFixture, HalfGridOnDeviceDataflow1DFBEach) {
    ...
    const auto tensor_spec = make_flat_dram_tensor_spec(entry_size, num_entries, DataType::UINT32);
    auto in_a = MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{});
    auto out_a = MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{});
    auto in_b = MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{});
    auto out_b = MeshTensor::allocate_on_device(*mesh_device, tensor_spec, TensorTopology{});
...
// Sort-shaped packing regression with a credit handshake on every bound DFB.
TEST_F(MeshDeviceFixture, SortShapeCoordinatorWorkerOnDevice) {
    auto& mesh_device = this->devices_.at(0);
    IDevice* device = mesh_device->get_devices()[0];
