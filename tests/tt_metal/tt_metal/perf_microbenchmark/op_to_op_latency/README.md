...
direction's NoC traffic *and* the math datapath (both imply `--bypass-compute`) — and skips output
validation, since nothing is copied.

`--bypass-compute` in turn forces `--lean-compute`, because the per-tile `TILE_IDX` stamp is gated by
`PROFILE_PER_TILE` rather than by `PASSTHROUGH`: under passthrough alone the kernel skips the copy but
still timestamps every tile, letting instrumentation pace the pipeline. So either single-direction flag
is sufficient on its own — passing `--lean-compute` alongside it is redundant, not required.

`--lean-compute` alone is **not** a substitute: it drops only instrumentation, so the compute kernel
still unpacks and packs every tile. Getting this wrong is expensive in both directions:

- **Write BW**: a "cheap read" (tiny reads, full-page pushes) still costs one DRAM read *transaction*
  per page — 1:1 with the writes, striding the same banks, so the controller pays read/write
  turnaround — and leaves the writer paced by the packer one tile at a time. Measured **33% low** on
  Blackhole (330 vs 440 GB/s at 110 cores). Pair `--write-only` with `--output-cb-depth-tiles >= 8`;
  the default 2 flushes after every write.
- **Read BW**: compute in the path back-pressures the reader through `cb_in` once it can't keep up.
  Harmless at high core counts, but at low core counts (high per-core BW) it turns the test into a
  compute measurement.

To sweep CB depths for peak BW, loop `--input-cb-depth-tiles` / `--output-cb-depth-tiles` from a
driver script (see `scripts/run_bw_vs_cores.sh`).
