...
protected:
    ...
    // Base FD test fuzzes the page size up to the requested value; stress variants fix it.
    virtual uint32_t paged_write_page_size_bytes(uint32_t requested_page_size) {
        return payload_generator_->get_random_size(MAX_XFER_SIZE_16B - 1, bytes_per_16B_unit, requested_page_size);
    }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        PagedWriteParams{4128, 10, DEFAULT_ITERATIONS_PAGED_WRITE, true},
